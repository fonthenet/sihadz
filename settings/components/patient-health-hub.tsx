...
export function PatientHealthHub({ userId, language = 'ar' }: PatientHealthHubProps) {
  ...
  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, { label: string; className: string }> = {
      ...
      fulfilled: { label: l.completed, className: 'bg-green-600 text-white' },
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="labResults" className="mt-6 space-y-4">
          {labRequests.length === 0 ? (
            ...
            labRequests.map(lab => (
              <Card key={lab.id} className={lab.status === 'fulfilled' ? 'border-green-500' : ''}>
                <CardContent className="p-6">
                  <div className="flex flex-col md:flex-row gap-4">
                    <div className="flex-1 space-y-3">
                      <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <div className="flex flex-col gap-2 min-w-[150px]">
                      <Button size="sm" variant="outline" onClick={() => setSelectedLabRequest(lab)}>
                        <Eye className="h-4 w-4 me-2" />
                        {l.viewDetails}
                      </Button>
