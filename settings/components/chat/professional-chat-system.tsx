...
export function ProfessionalChatSystem({
  ...
  // Scroll behavior + incoming notifications
  useEffect(() => {
    ...
    if (last.sender_id !== userId && last.id !== lastNotifiedMessageIdRef.current) {
      lastNotifiedMessageIdRef.current = last.id
      playNotification()
    }

    const shouldScroll = last.sender_id !== userId || stickToBottomRef.current
      ...
      <div className={cn('flex', mine ? 'justify-end' : 'justify-start')}>
        <div className={cn('max-w-[84%] space-y-1')}>
          ...
          >
            ...
            {/* Content */}
            {m.is_deleted ? (
              <div className={cn('text-sm italic', mine ? 'text-primary-foreground/70' : 'text-muted-foreground')}>
                Message deleted
              </div>
            ) : m.content ? (
