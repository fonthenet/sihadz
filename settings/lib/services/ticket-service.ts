...
// Get ticket timeline
export async function getTicketTimeline(ticketId: string): Promise<{ entries: TicketTimelineEntry[]; error: any }> {
  const supabase = createBrowserClient()
...
// Get ticket messages
export async function getTicketMessages(ticketId: string): Promise<{ messages: TicketMessage[]; error: any }> {
  const supabase = createBrowserClient()
