...
export default function DoctorsManagementPage() {
  ...
  const handleVerifyDoctor = async (doctorId: string) => {
    const supabase = createBrowserClient()
    try {
      console.log('[v0] Verifying doctor:', doctorId)
      ...
      if (error) {
        console.error('[v0] Error verifying doctor:', error)
        alert(`Failed to verify doctor: ${error.message}`)
        throw error
      }
      
      console.log('[v0] Doctor verified successfully:', data)
