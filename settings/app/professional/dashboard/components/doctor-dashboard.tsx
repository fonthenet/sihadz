...
import { 
  Calendar, Users, DollarSign, Star, Clock, Bell, Settings, LogOut,
  FileText, Pill, FlaskConical, Send, Plus, Search, CheckCircle, XCircle,
  Stethoscope, ClipboardList, MessageSquare, AlertCircle, Trash2
} from 'lucide-react'
...
export default function DoctorDashboard({ professional, profile, onSignOut }: DashboardProps) {
  ...
  const loadDashboardData = async () => {
    ...
    // First, check if there's a linked doctors table entry
    ...
    const doctorId = doctorEntry?.id || professional.id
    ...
    // Load recent lab requests
    const { data: labList, error: labError } = await supabase
      .from('lab_test_requests')
      .select('*, patient:profiles!lab_test_requests_patient_id_fkey(full_name, phone)')
      .eq('doctor_id', doctorId)
      .order('created_at', { ascending: false })
      .limit(10)

    console.log('[v0] Lab requests loaded:', labList, 'Error:', labError)
    ...
    setStats({
      ...
      pendingLabRequests: labList?.filter((l: any) => l.status === 'pending').length || 0,
    })
  }
  ...
  const handleCreateLabRequest = async () => {
    ...
    const requestNumber = `LAB-${Date.now()}`
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <Card className="mb-6 border-primary/20 bg-gradient-to-r from-primary/5 to-indigo-500/5">
                  ...
                  <div className="space-y-4">
                    {labRequests.map((req) => (
                      <div key={req.id} className="flex items-center justify-between p-4 border rounded-lg">
                        <div>
                          <p className="font-medium">{req.profiles?.full_name || 'Patient'}</p>
