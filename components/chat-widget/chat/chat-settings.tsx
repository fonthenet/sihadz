...
import { 
  ...
  Shield, UserX, ChevronRight, Move,
  MessageCircle, Stethoscope, User
} from 'lucide-react'
        ...
        <div className="max-h-[70vh] overflow-y-auto">
          {activeSection === 'main' ? (
            <div className="p-4 space-y-6">
              ...
              <SettingsSection title="Availability" icon={<MessageCircle className="h-4 w-4" />}>
                <SettingToggle
                  label="Accept New Chats"
                  description="Allow new conversations from others"
                  checked={settings.accepting_new_chats !== false}
                  onChange={(v) => handleToggle('accepting_new_chats', v)}
                />
                <SettingToggle
                  label="Chat with Professionals"
                  description="Allow doctors, pharmacies, and other pros to message you"
                  checked={settings.accept_from_providers !== false}
                  onChange={(v) => handleToggle('accept_from_providers', v)}
                  icon={<Stethoscope className="h-4 w-4" />}
                />
                <SettingToggle
                  label="Chat with Patients"
                  description="Allow patients to message you"
                  checked={settings.accept_from_patients !== false}
                  onChange={(v) => handleToggle('accept_from_patients', v)}
              ...
              <SettingsSection title="Notifications" icon={<Bell className="h-4 w-4" />}>
                ...
                <SettingToggle
                  ...
                  description="Show browser notifications when minimized"
                  ...
                  onChange={async (v) => {
                    ...
                    handleToggle('desktop_notifications', v)
                  }}
              ...
              {/* Position */}
              {onResetPosition && (
                <SettingsSection title="Position" icon={<Move className="h-4 w-4" />}>
                  <p className="p-3 rounded-xl text-sm text-slate-600 dark:text-slate-400">
                    Drag the chat widget by the grip icon or the launcher button to move it.
                  </p>
                  <button
                    onClick={() => { onResetPosition(); onClose() }}
                    className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 text-sm font-medium text-slate-700 dark:text-slate-300"
                  >
