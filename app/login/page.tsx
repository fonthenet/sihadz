...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        // Determine account type first – professionals never go to patient booking/dashboard
        let redirectPath = '/dashboard'

        try {
          // 1. Check professionals table first (doctors, pharmacies, labs, clinics)
          ...
          if (professional) {
            // Pro accounts cannot book – always redirect to professional dashboard
            redirectPath = '/professional/dashboard?from=patient-login'
          } else {
            // 2. Fallback: check profiles.user_type (super_admin, legacy professional markers)
            ...
            if (!profileError && profile) {
              ...
              if (userType === 'super_admin' || userType === 'admin') {
                ...
                // Patient: honor redirect param (e.g. booking) if safe
                if (redirectTo && typeof redirectTo === 'string' && redirectTo.startsWith('/') && !redirectTo.startsWith('//')) {
                  redirectPath = redirectTo
                }
              }
            } else {
              // Patient (no profile): honor redirect param if safe
              if (redirectTo && typeof redirectTo === 'string' && redirectTo.startsWith('/') && !redirectTo.startsWith('//')) {
                redirectPath = redirectTo
              }
            }
          ...
          console.error('Account type check failed, using default redirect:', err)
        }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-muted/70" dir={dir}>
      {/* Header */}
      ...
      <div className="flex-1 flex items-center justify-center p-4 overflow-y-auto">
        <Card className="w-full max-w-md max-h-[calc(100vh-6rem)] overflow-y-auto bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/60 border border-border/50 shadow-xl">
