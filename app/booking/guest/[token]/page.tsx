    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto max-w-2xl px-4 py-8">
        ...
        <InfoCard
          ...
          className="mb-6"
          items={[
            { label: language === 'ar' ? 'التاريخ' : 'Date', value: formatDate(appointment.appointment_date), icon: Calendar },
            { label: language === 'ar' ? 'الوقت' : 'Time', value: appointment.appointment_time, icon: Clock },
            {
              label: language === 'ar' ? 'نوع الزيارة' : 'Visit Type',
              value: appointment.visit_type === 'e-visit'
                ? (language === 'ar' ? 'زيارة إلكترونية' : 'E-Visit / Video Consultation')
                : (language === 'ar' ? 'زيارة شخصية' : 'In-Person Visit'),
              icon: appointment.visit_type === 'e-visit' ? Video : Building,
            },
          ]}
        />
          ...
          <Card className="mb-6">
            <CardContent className="pt-4">
              <p className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider flex items-center gap-1 mb-1">
                <FileText className="h-3.5 w-3.5" /> {language === 'ar' ? 'الأعراض' : 'Symptoms'}
              </p>
