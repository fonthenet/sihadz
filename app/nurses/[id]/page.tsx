...
const labels = {
  ar: {
    ...
    notFound: 'الممرض/ة غير موجود',
    appointmentsAutoConfirmed: 'المواعيد مؤكدة تلقائياً',
    appointmentsSubjectToConfirmation: 'المواعيد تخضع للتأكيد',
  },
  fr: {
    ...
    reviews: 'Avis',
    verified: 'Infirmier/ère vérifié(e)',
    ...
    loading: 'Chargement...',
    notFound: 'Infirmier/ère introuvable',
  ...
  en: {
    ...
    appointmentsSubjectToConfirmation: 'Appointments subject to confirmation',
  },
}
...
export default function NurseProfilePage() {
  ...
  const { isFavorite } = useFavorites('nurse')
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!proErr && pro) {
        ...
        setNurse({
          id: pro.id,
          name: (pro.business_name as string) || 'Nurse',
          nameAr: (pro.business_name_ar as string) || (pro.business_name as string) || 'ممرض/ة',
          ...
          price: Number(pro.consultation_fee) || 2000,
          ...
          workingHours: wh,
          supportsEVisit: !!(pro as { supports_e_visit?: boolean }).supports_e_visit ?? true,
          canBook: true,
  ...
  const displayName = lang === 'ar' ? nurse?.nameAr : nurse?.name
  const displaySpecialty = lang === 'ar' ? nurse?.specialtyAr : nurse?.specialty
      ...
      <div className="min-h-screen bg-background flex flex-col" dir={dir}>
        ...
        <main className="flex-1 container mx-auto px-4 py-6">
          ...
          <Button asChild variant="outline" className="mt-4">
            <Link href="/search">{lang === 'ar' ? 'البحث عن ممرضين' : lang === 'fr' ? 'Rechercher des infirmiers' : 'Search nurses'}</Link>
          </Button>
        </main>
        <Footer />
      </div>
      ...
      <main className="flex-1 container mx-auto px-4 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 max-w-4xl mx-auto">
          <div className="lg:col-span-6 space-y-4">
              ...
              <CardContent className="p-5">
                <div className="flex gap-4">
                  <Avatar className="h-24 w-24 rounded-full ring-2 ring-primary/20 shrink-0">
                    <AvatarImage src={nurse.avatarUrl || undefined} alt="" className="object-cover" />
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <FavoriteButton
                        ...
                        initialFavorited={isFavorite(nurse.id)}
                        size="default"
                      />
                    </div>
                    <div className="flex items-center gap-1.5 mt-2">
                      <Star className="h-5 w-5 fill-yellow-400 text-yellow-400" />
                      <span className="text-base font-medium">{nurse.rating}</span>
                    ...
                    <div className="flex flex-wrap gap-2 mt-3">
                      <Badge variant="outline" className="text-sm px-2.5 py-1">
                        ...
                        {nurse.price.toLocaleString()} {l.dzd}
                      </Badge>
                      {nurse.supportsEVisit && nurse.eVisitPrice != null && (
                        <Badge variant="outline" className="text-sm px-2.5 py-1 border-primary text-primary">
                          <Video className="h-3.5 w-3.5 me-1" />
                          {nurse.eVisitPrice.toLocaleString()} {l.dzd}
                        </Badge>
                      )}
                    </div>
                    <p className="flex items-center gap-1.5 mt-3 text-sm text-muted-foreground">
                      <CalendarCheck className="h-4 w-4 shrink-0" />
                      {nurse.autoConfirmAppointments ? l.appointmentsAutoConfirmed : l.appointmentsSubjectToConfirmation}
                    </p>
                  </div>
                </div>
              </CardContent>
            ...
            <ProfileServicesSection professionalId={nurse.id} className="shrink-0" />
            ...
            <div className="shrink-0">
              <Card className="mt-3">
                <CardContent className="p-4">
                  ...
                  {nurse.workingHours.length > 0 ? (
                    <ul className="space-y-1 text-sm text-muted-foreground">
                      {nurse.workingHours.map((hours, idx) => (
                        <li key={idx}>{hours}</li>
                      ))}
                    </ul>
                  ) : (
              ...
              <CardContent className="space-y-2 pt-0 px-4 pb-4 overflow-hidden">
                {nurse.canBook !== false ? (
                  ...
                  <p className="text-sm text-muted-foreground text-center py-4">
                    {lang === 'ar' ? 'لحجز موعد، اتصل بالممرض/ة' : lang === 'fr' ? 'Pour réserver, contactez l\'infirmier/ère' : 'To book, please contact the nurse'}
                  </p>
                )}
                  ...
                  {nurse.canBook !== false ? (
                    <>
                      <Button asChild className="w-full" disabled={!selectedDate || !selectedTime}>
                        <Link href={`/booking/new?nurse=${nurse.id}&date=${selectedDate}&time=${selectedTime}&type=in-person`}>
                          <Building className="h-4 w-4 me-2" />
                          {l.bookAppointment}
                        </Link>
                      </Button>
                      {nurse.supportsEVisit && (
                        <Button
                          asChild
                          variant="outline"
                        ...
                        >
                          <Link href={`/booking/new?nurse=${nurse.id}&date=${selectedDate}&time=${selectedTime}&type=e-visit`}>
