...
export async function POST(request: NextRequest) {
  try {
    ...
    // Use request origin so redirects go to current site (sihadz.com), not localhost
    let baseUrl = getRequestOrigin(request)
    if (baseUrl.includes('localhost') && process.env.NEXT_PUBLIC_SITE_URL) {
      baseUrl = process.env.NEXT_PUBLIC_SITE_URL.replace(/\/$/, '')
    } else if (baseUrl.includes('localhost') && process.env.VERCEL_URL) {
    ...
    const checkout = await createCheckout({
      amount: amountInCentimes,
      description: description || 'SihaDZ Payment',
