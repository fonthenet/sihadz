...
export default function PharmacySelector({ open, onClose, onSelect, patientId, doctorId }: PharmacySelectorProps) {
  ...
  // Use location hook for auto-detect
  ...
  /** Load doctor's recently used pharmacies from prescriptions */
  const loadRecentlyUsed = async () => {
    if (!doctorId) return
    // Get distinct pharmacy IDs from recent prescriptions sent by this doctor
    ...
    if (data) {
      // Get unique pharmacy IDs in order of most recent use
      ...
      setRecentlyUsedIds(uniqueIds.slice(0, 5)) // Keep top 5
    }
  }
  ...
  const loadPharmacies = async () => {
    ...
    // SINGLE SOURCE OF TRUTH: Load from professionals table with more details
    let query = supabase
      .from('professionals')
    ...
    if (error) {
      ...
      // Map professionals fields; keep business_name so "Sent to X" and APIs work
  ...
  // Sort pharmacies: favorites first, then recently used, then rest
  ...
  // Separate recently used for display
  ...
  /** Render a pharmacy card with full details */
