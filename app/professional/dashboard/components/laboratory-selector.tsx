...
export default function LaboratorySelector({ open, onClose, onSelect, patientId, doctorId }: LaboratorySelectorProps) {
  ...
  /** Load doctor's recently used laboratories from lab requests */
  const loadRecentlyUsed = async () => {
    if (!doctorId) return
    // Get distinct laboratory IDs from recent lab requests sent by this doctor
    ...
    if (data) {
      // Get unique lab IDs in order of most recent use
  ...
  const loadLaboratories = async () => {
    setLoading(true)
    // Same pattern as pharmacy-selector: professionals table with more details
    let query = supabase
      .from('professionals')
  ...
  // Sort laboratories: favorites first, then recently used, then rest
  ...
  /** Render a laboratory card with full details */
