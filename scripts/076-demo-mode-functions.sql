-- ============================================================================
-- DEMO MODE FUNCTIONS
-- Callable via Supabase RPC for demo switch (seed/clear pharmacy test data)
-- Requires service role or authenticated pharmacy owner.
-- ============================================================================

-- Ensure prescriptions has total_price (idempotent)
ALTER TABLE prescriptions ADD COLUMN IF NOT EXISTS total_price DECIMAL(12,2);

-- Check if demo/seed data exists (for switch state)
CREATE OR REPLACE FUNCTION demo_has_seed_data()
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  RETURN EXISTS (
    SELECT 1 FROM pos_sales WHERE sale_number LIKE 'SEED-%' LIMIT 1
  ) OR EXISTS (
    SELECT 1 FROM chifa_invoices WHERE invoice_number LIKE 'SEED-%' LIMIT 1
  ) OR EXISTS (
    SELECT 1 FROM accounting_journal_entries WHERE entry_number LIKE 'SEED-%' LIMIT 1
  );
END;
$$;

-- Clear all demo/seed data
CREATE OR REPLACE FUNCTION demo_clear_pharmacy_data()
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  v_result := v_result || jsonb_build_object('journal_lines', v_deleted);

  WITH d AS (DELETE FROM accounting_journal_entries WHERE entry_number LIKE 'SEED-%' RETURNING 1)
  SELECT COUNT(*) INTO v_deleted FROM d;
  v_result := v_result || jsonb_build_object('journal_entries', v_deleted);

  WITH d AS (DELETE FROM chifa_rejections WHERE rejection_motif LIKE 'SEED%' RETURNING 1)
  SELECT COUNT(*) INTO v_deleted FROM d;
  v_result := v_result || jsonb_build_object('rejections', v_deleted);
  ...
  SELECT COUNT(*) INTO v_deleted FROM d;
  v_result := v_result || jsonb_build_object('invoices', v_deleted);
  ...
  SELECT COUNT(*) INTO v_deleted FROM d;
  v_result := v_result || jsonb_build_object('pos_sales', v_deleted);
...
-- Seed demo data for all pharmacies (wraps 074 logic)
CREATE OR REPLACE FUNCTION demo_seed_pharmacy_data()
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      IF v_auth_user_id IS NULL THEN SELECT id INTO v_auth_user_id FROM profiles LIMIT 1; END IF;
      ...
      ON CONFLICT (pharmacy_id, session_number) DO NOTHING RETURNING id INTO v_session_id;
      ...
      ON CONFLICT (pharmacy_id, session_number) DO NOTHING RETURNING id INTO v_session_closed_id;
      ...
      FOR v_i IN 1..5 LOOP
        ...
        VALUES (v_pharmacy.id, v_session_closed_id, v_drawer_id, v_warehouse_id, 'SEED-TICKET-' || v_i, v_customer_id, 45000 + v_i * 18000, 45000 + v_i * 18000, 45000 + v_i * 18000, 'completed')
        ON CONFLICT (pharmacy_id, sale_number) DO NOTHING RETURNING id INTO v_sale_id;
      ...
      VALUES (v_pharmacy.id, v_session_closed_id, v_drawer_id, v_warehouse_id, 'SEED-TICKET-CHIFA', v_customer_id, 85000, 85000, 17000, 68000, 17000, 'completed')
      ...
      IF v_fiscal_year_id IS NOT NULL AND v_journal_type_id IS NOT NULL AND v_account_531 IS NOT NULL AND v_account_700 IS NOT NULL THEN
        ...
        ON CONFLICT (pharmacy_id, entry_number) DO NOTHING RETURNING id INTO v_sale_id;
        IF v_sale_id IS NOT NULL THEN
          INSERT INTO accounting_journal_lines (entry_id, line_number, account_id, account_code, description, debit_amount, credit_amount)
          VALUES (v_sale_id, 1, v_account_531, '531', 'Caisse', 285000, 0), (v_sale_id, 2, v_account_700, '700', 'Ventes', 0, 285000);
        END IF;
        ...
        IF v_sale_id IS NOT NULL AND v_account_4113 IS NOT NULL AND v_account_512 IS NOT NULL THEN
          INSERT INTO accounting_journal_lines (entry_id, line_number, account_id, account_code, description, debit_amount, credit_amount)
          VALUES (v_sale_id, 1, v_account_512, '512', 'Banque', 156000, 0), (v_sale_id, 2, v_account_4113, '4113', 'Clients CNAS', 0, 156000);
        END IF;
        ...
        IF v_sale_id IS NOT NULL THEN
          INSERT INTO accounting_journal_lines (entry_id, line_number, account_id, account_code, description, debit_amount, credit_amount)
          VALUES (v_sale_id, 1, v_account_531, '531', 'Caisse', 125000, 0), (v_sale_id, 2, v_account_700, '700', 'Ventes', 0, 125000);
        END IF;
      END IF;
      ...
      IF v_supplier_id IS NOT NULL THEN
        ...
        ON CONFLICT (pharmacy_id, po_number) DO NOTHING RETURNING id INTO v_po_id;
      ...
      ON CONFLICT (pharmacy_id, transfer_number) DO NOTHING RETURNING id INTO v_transfer_id;
    ...
    EXCEPTION WHEN OTHERS THEN
      RAISE WARNING 'Error for pharmacy %: %', v_pharmacy.id, SQLERRM;
    END;
  END LOOP;

  RETURN jsonb_build_object('success', true, 'pharmacies_seeded', v_seeded);
END;
$$;

-- Grant execute to service role (used by API)
GRANT EXECUTE ON FUNCTION demo_has_seed_data() TO service_role;
GRANT EXECUTE ON FUNCTION demo_clear_pharmacy_data() TO service_role;
GRANT EXECUTE ON FUNCTION demo_seed_pharmacy_data() TO service_role;
